...
import { Cpu, ShieldAlert, AlertCircle, Sparkles, RefreshCw, Volume2 } from 'lucide-react';
                ...
                <div className="fixed inset-0 z-[6000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md transition-all duration-500">
                    <div className={`relative w-full max-w-md rounded-[2.5rem] p-8 border backdrop-blur-2xl transition-all duration-500 overflow-hidden ${isDark ? 'bg-[#0D0D0F]/70 border-white/10 shadow-[0_0_40px_rgba(0,0,0,0.5)]' : 'bg-white/70 border-white/40 shadow-2xl shadow-slate-200/50'}`}>
                        ...
                        <div className="flex gap-3 relative z-10">
                            <button onClick={() => setConflictData(null)} className={`flex-1 py-4 rounded-xl uppercase font-black text-[10px] tracking-widest transition-all backdrop-blur-xl ${isDark ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-white/50 text-slate-900 hover:bg-white/80'}`}>Back</button>
                            <button onClick={() => confirmSave(conflictData.original)} className="flex-1 py-4 bg-rose-600 text-white rounded-xl uppercase font-black text-[10px] tracking-widest hover:bg-rose-700 hover:shadow-lg transition-all">Ignore</button>
                        </div>
                    </div>
                </div>
            )}
